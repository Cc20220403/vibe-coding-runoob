    ...
    <>
      ...
      >
        ...
        <div className="flex items-start gap-3">
          ...
          {/* 标题（flex-1，自动截断） */}
          <h3
            className={`flex-1 min-w-0 text-sm font-medium transition-colors ${isDone ? 'line-through text-slate-400/70 dark:text-slate-500/70' : 'text-slate-800 dark:text-slate-100'}`}
          >
            {task.title}
          </h3>

          {/* 右侧：优先级 + 日期 + 编辑 + 删除（并列在右端） */}
        ...
        {(task.category || task.description) && (
          <div className="flex items-start gap-2 mt-1.5 pl-[30px]">
            {task.category && (
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-indigo-500/8 text-indigo-600/80 dark:bg-indigo-400/10 dark:text-indigo-400/80 border border-indigo-200/30 dark:border-indigo-400/10 shrink-0">
                {task.category}
              </span>
            )}
            {task.description && (
              <p className="text-xs text-slate-500/70 dark:text-slate-400/70 line-clamp-2">
                {task.description}
              </p>
            )}
          </div>
